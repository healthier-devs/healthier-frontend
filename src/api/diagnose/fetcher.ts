...
export const diagnosisFetcher = {
  getQuestions(diagnosisType: string, gender: string): Promise<IQuestionResponse> {
    ...
    return fetcher.get(`/${diagnosisType}?gender=${gender}`);
  },
  postAnswers(diagnosisType: string, postAnswersBody: IPostAnswersBody): Promise<IDiagnoseResponse> {
    return fetcher.post(`/${diagnosisType}`, postAnswersBody);
    // if (diagnosisType === SYMPTOMS_TYPES_MAP.stomach) {
